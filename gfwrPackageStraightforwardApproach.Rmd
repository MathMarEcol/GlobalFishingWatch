...
We have the same parameters than the `get_raster` function, plus one parameter `group_by_vars` which is basically the choice of the variable(s) we want to group by the data if we want to summarize the fishing hours per specific variables. <br> 
...
```{r}
get_gfwData <- function(region, start_date, end_date, temp_res, spat_res, key, sfcrs, group_by_vars = NULL) {
...
```{r, message=FALSE}
data_sf_combined <- get_gfwData('Australie', "2019-01-01", "2023-12-31", "yearly", "low", key)
```
