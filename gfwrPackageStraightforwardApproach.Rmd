...
:   \<[Shortened URL](https://tinyurl.com/2pr5nxxk)\> <br> 2. Request
...
```{r, results='hide'}
region_id <- get_region_id(region_name = 'Australia', 
                           region_source = 'eez', 
                           key = key)$id
```
...
get_gfwData <- function(region, start_date, end_date, temp_res,
  ...
  region_id <- gfwr::get_region_id(region_name = region,
                                   region_source = 'eez',
                                   key = key)$id[1]
  ...
  if (as.numeric(difftime(end_date, start_date, units = "days")) <= 366) {
    ...
    data_df <- purrr::map_dfr(date_chunks, 
...
```{r, message=FALSE}
data_sf_combined <- get_gfwData(region = 'Australie', 
...
main_plot <- ggplot(data_sf_combined) +
  ...
  theme_minimal() +
  labs(title = "2022 Vessel Activity Map", 
...
overlay_plot <- ggplot(data_sf_combined) +
  ...
  scale_color_viridis_c(guide = "legend") +
  labs(title = "Vessel Activity Map in Australia between 2019 and 2023", 
...
ggplot(data_sf_combined) +
  ...
  labs(title = "Vessel Activity Map in Australia between 2019 and 2024", 
       subtitle = "Fishing Hours data recorded by GFW", 
       color = "Years") +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 8),  
...
```{r, message=FALSE}
data_sf_combined <- get_gfwData('Micronesia', 
                                "2022-01-01", 
                                "2022-12-31", 
                                "monthly", 
                                "low", 
...
# Check and modify if necessary the spatial reference of data_sf_combined
data_sf_combined <- st_set_crs(data_sf_combined, 
                               st_crs(rnaturalearth::ne_coastline(scale = 'large'))) 
...
eezs <-  mregions::mr_shp(key = "Micronesia:eez") %>% 
  dplyr::filter(eezs, geoname ==  'Micronesian Exclusive Economic Zone') %>%
  sf::st_transform(crs = sf::st_crs(data_sf_combined)) %>%
  sf::st_make_valid() %>%
  sf::st_intersection(sf::st_as_sfc(sf::st_bbox(data_sf_combined)))
```
...
```{r, echo=FALSE, message=FALSE, results='hide'}
data_sf_combined <- get_gfwData('Papua New Guinea', 
                                "2022-01-01", 
                                "2022-12-31", 
                                "monthly", 
                                "low", 
