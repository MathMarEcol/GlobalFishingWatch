...
Date range is limited to 1-year. Nevertheless, with some modifications, we can get round these problems through `get_gfwFishingEffort`. We have the same parameters than the `get_raster` function, plus one parameter `group_by_vars` which is basically the choice of the variable(s) we want to group by the data if we want to summarize the fishing hours per specific variables. <br> 
Different possible values can be combined and are : <br>
- `Time Range`, `Flag`, `Geartype`. <br>
_(A combination can be : c('Time Range','Geartype'), if you want to get the sum of fishing hours per date and geartype, for example you want to display the drifting longline fishing in a specific year)_ 
```{r}
get_gfwData <- function(region, start_date, end_date, temp_res, spat_res, key, group_by_vars = NULL) {
  ...
  if (temp_res == "monthly") {
  data_sf <- data_sf %>%
    separate("Time Range", into = c("Year", "Month"), sep = "-", remove = FALSE)}

  if (temp_res == "daily") {
  data_sf <- data_sf %>%
    separate("Time Range", into = c("Year", "Month", "Day"), sep = "-", remove = FALSE)}
...
```{r, message=FALSE}
data_sf_combined <- get_gfwData('Australie', "2019-01-01", "2023-01-01", "monthly", "low", key)
```
...
ggplot(data_sf_combined) +
  ...
  labs(title = "Vessel Activity Map in Australia between 2019 and 2023", subtitle = "Fishing Hours data recorded by GFW", color = "Years") +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 8),  
        legend.title = element_text(size = 10))  
```
...
We need to change the temporal range and add the variable to group to display the total fishing hours per month. <br>
...
```{r, message=FALSE}
data_sf_combined <- get_gfwData('Australie', "2019-01-01", "2023-01-01", "monthly", "low", key, "Time Range")
```

```{r, echo=FALSE}
ggplot(data_sf_combined, aes(x = Month, y = `Apparent Fishing Hours`, color = Year, group = Year)) +
  geom_line() +
  geom_point() +
  labs(title = "Total Fishing Hours per month (2019-2023)", 
...
```{r, message=FALSE}
data_sf_combined <- get_gfwData('Micronesia', "2019-01-01", "2023-01-01", "monthly", "low", key)
```
...
```{r, echo=FALSE, message=FALSE, results='hide'}
data_sf_combined <- get_gfwData('Papua New Guinea', "2019-01-01", "2023-01-01", "monthly", "low", key)
